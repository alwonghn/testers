...
public class QOS extends JPanel implements MouseListener {
    ...
    private ArrayList<RunningVehicleBean> alDir1RunningVeh;
    private ArrayList<RunningVehicleBean> alDir2RunningVeh;
    private ArrayList<RunningVehicleBean> alAllRunningVeh;
...
*/
        ...
        for(String s: alSvc){
            ...
            int sInt = Integer.parseInt(s);
//            HeadwayBean headwayList = wsport.headwayList(sInt);
            HeadwayBean headwayList = mgr.getHeadway(sInt);
            alDir1RunningVeh = (ArrayList)headwayList.getDir1RunningVehicle();
            alDir2RunningVeh = (ArrayList)headwayList.getDir2RunningVehicle();
            alAllRunningVeh = new ArrayList<RunningVehicleBean>();
            for(RunningVehicleBean rv1 : alDir1RunningVeh){
                alAllRunningVeh.add(rv1);
            }
            for(RunningVehicleBean rv2 : alDir2RunningVeh){
                alAllRunningVeh.add(rv2);
            }
            int totRunVeh = alAllRunningVeh.size();
            svcAdt.setDeployed(totRunVeh);
            for(RunningVehicleBean rv : alAllRunningVeh){
                int hwdev = rv.getScheduled_headway()-rv.getActual_headway();
                if(hwdev <= -5){
                    svcAdt.addOneSvcHw5n();
                }
                if(hwdev >= +5){
                ...
                double svcLoading = rv.getPassenger_count() / rv.getPassenger_capacity();
                if(svcLoading >= 0.95){
                    svcAdt.addOneSvcLoadg95();
                }
                if(svcLoading >= 0.93 && svcLoading < 0.95){
